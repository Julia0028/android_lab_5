...
Espresso тест:
Сначала с помощью теста в EditText записывается слово "Hello". Далее проверяется нажатие кнопки и проверка измении текста на ней. Далее меняем ориентацию устройства: идет проверка на сохранение слова в EditText и на текст кнопки (он сохраняться не должен).  
